...
<?php
    ...
    echo '<div class="mikio-page-fill">';
        ...
        if($TEMPLATE->getConf('footerInPage') == TRUE && ($ACT=='show' || $ACT=='denied')) $TEMPLATE->includeFooter();
        echo '</div>';
        if($rightsidebar != '') echo '<aside class="mikio-sidebar mikio-sidebar-right"></aside>';
    echo '</div>';

    if($TEMPLATE->getConf('footerInPage') == FALSE && ($ACT=='show' || $ACT=='denied')) $TEMPLATE->includeFooter();
